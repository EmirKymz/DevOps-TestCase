...
# Adım 7: Bash Script
- Jenkins pipeline'ı otomatik olarak çalıştırmak için bir bash script oluşturdum. Bu script, Jenkins pipeline'ı çalıştırmak için bir curl komutu kullanır. Bu curl komutu, Jenkins sunucusuna bir HTTP POST isteği gönderir ve Jenkins pipeline'ı çalıştırır. Bu scripti aşağıdaki adımları uygulayarak oluşturabilirsiniz.
